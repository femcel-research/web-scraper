...
class ArchiveToContent:
    ...
    def get_thread_id(self) -> str:
        ...
        try:
            thread_id = self.thread_soup.find(class_=self.id_class).get("id")
        except:
        ...
        if thread_id is None:
            try:
                thread_id = (
                    self.thread_soup.find(class_=self.id_class)
                    .find("a")
                    .get("id")
                    .replace("post_no_", "")
                    .replace("op_", "")
            ...
            thread_id = thread_id.replace("op_", "").replace("t", "")
            return thread_id
    ...
    def get_post_date(self, post_tag: Tag) -> str:
        ...
        try:
            ...
            if date_posted and "datetime" in date_posted.attrs:
                date_string = date_posted["datetime"]
                self.logger.debug(f"Found date in <time> tag: {date_string}")

            else:
                self.logger.error("Post date found, but empty")
                self.logger.debug("Attempting to find date using class 'dateTime'")
                datetime_class_tag = post_tag.find(class_="dateTime")
                if datetime_class_tag:
                    # Split the texts: expecting two parts (date and time)
                    parts = datetime_class_tag.get_text(strip=True).split()
                    if len(parts) >= 2:
                        date_string = f"{parts[0]}T{parts[1]}".replace("Post", "Z")
                        self.logger.debug(
                            f"Found date in class 'dateTime': {date_string}"
                        )
                    else:
                        self.logger.warning(
                            f"Unexpected date format: {' '.join(parts)}"
                        )
                else:
                    self.logger.error(
                        "Date not found in <time> or with class 'dateTime'."
                    )
    ...
    def get_post_id(self, post_tag: Tag) -> str:
        ...
        try:
            ...
            try:
                post_id: str = post_tag.find(class_=self.id_class).get("id")
                if post_id is None:
                    ...
                    if post_id is None:
                        ...
                        post_id = post_id.replace("p", "")  # String cleaning
                        return post_id
            # If the 'id' tag doesn't exist, then catch the Exception
                ...
                if post_id is None:
                    ...
                    post_id = post_id.replace("p", "")
                    return post_id
            else:
                self.logger.debug(f"Post ID successfully found: {post_id}")
                post_id = post_id.replace("p", "")
                return post_id
        except Exception as error:
    ...
    def get_post_content(self, post_tag: Tag) -> str:
        ...
        try:
            ...
            if post_body:
                ...
                self.logger.debug("Attempting to find body using class 'postMessage'")
                post_message: Tag = post_tag.find(class_="postMessage")
                if post_message:
                    post_text: str = post_message.get_text("\n", True)
                    self.logger.debug("Post content successfully found")
                else:
                    self.logger.error(f"Post content (body) found, but empty")
    ...
    def get_thread_image_link(self) -> str:
        ...
        try:
            ...
            if img_tag is None:
                self.logger.warning(
                    "No img tag found. " "No thread image link available."
                )
                return ""
    ...
    def get_post_replied_to_ids(self, post_tag: Tag) -> list[str]:
        ...
        try:
            ...
            if post_body is None:
                post_body = post_tag.find(class_="postMessage")
            links_to_other_posts: str
            links_to_other_posts = post_body.find_all(self._has_href_and_onclick_a)
            post_links: list[str] = []
    ...
    def get_all_post_data(self, op: Tag, replies: list[Tag]) -> dict:
        ...
        try:
            ...
            for reply in replies:
                all_post_data[f"reply_{self.get_post_id(reply)}".replace("p", "")] = (
    ...
    def get_replies(self, replies: list[Tag]) -> dict:
        ...
        try:
            ...
            self.logger.debug("Data from every reply successfully collected")
